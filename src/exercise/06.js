...
function useControlledSwitchWarning(isControlled) {
  ...
  React.useEffect(() => {
    warning(
      ...
      'Changing from not controlled to controlled',
    )
    warning(
      !(!isControlled && isControlledRef.current),
...
function useToggle({
  ...
  function dispatchWithOnChange(action) {
    if (!onIsControlled) {
      warning(
        onChange && !readonly,
        'Providing the controlled on without passing an on change handler',
      )
      dispatch(action)
    }
    if (onChange) {
      warning(
        onIsControlled,
        'Providing an on change handler without being a controlled component',
      )
